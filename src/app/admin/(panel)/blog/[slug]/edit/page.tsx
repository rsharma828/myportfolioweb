...
import { isProbablyHtml } from "@/lib/html-content";
...
export default async function EditBlogPage({ params }: Props) {
  ...
  const publishedLocal = dateToDatetimeLocalValue(post.publishedAt);
  const assetBaseUrl = getPublicAssetBaseUrl();
  const bodyForEditor = isProbablyHtml(post.body) ? post.body : await markdownToHTML(post.body);
      ...
      <form action={upsertBlogPostAction} className="pb-24">
        ...
        <div className="mx-auto w-full max-w-[680px] px-4 pt-10 md:px-6">
          ...
          <p className="mt-6 text-center text-xs text-muted-foreground/70">
            If you skip the cover above, the first image in your post is used for listings and previews.
          </p>
        </div>
          ...
          <details className="group rounded-xl border border-border/80 bg-background/60 p-4 shadow-sm">
            ...
            <div className="mt-6 space-y-5 border-t border-border/60 pt-6">
              <div className="space-y-2">
                ...
                <Input id="slug" name="slug" required defaultValue={post.slug} className="bg-background" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="publishedAt">Publish date</Label>
                <Input id="publishedAt" name="publishedAt" type="datetime-local" defaultValue={publishedLocal} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="tags">Tags</Label>
                <Input id="tags" name="tags" defaultValue={post.tags.join(", ")} className="bg-background" />
              </div>
